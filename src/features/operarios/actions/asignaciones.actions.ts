...
export async function reportarAvanceAction(data: { asignacion_id: string; piezas_reportadas: number; piezas_buenas?: number; piezas_defectuosas?: number; maquina_id?: string; notas?: string; fecha_inicio?: string }): Promise<any> {
  const token = await getToken();
