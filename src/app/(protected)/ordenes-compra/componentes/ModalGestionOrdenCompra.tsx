...
export function ModalGestionOrdenCompra({ ordenEditando, onClose, readOnly = false }: { onClose: () => void; ordenEditando?: OrdenCompra; readOnly?: boolean; }) {
                ...
                <div className="flex items-center justify-between p-5 border-b border-[#1e2130]">
                    <div>
                        <h2 className="text-xl font-black text-white">
                            {readOnly ? "Detalles de la Orden de Compra" : isEdit ? "Editar Orden de Compra" : "Nueva Orden de Compra"}
                        </h2>
                        <p className="text-sm text-[#94a3b8] mt-1">
                            {readOnly ? "Vista detallada de la orden." : isEdit ? "Modifique los datos de la orden." : "Ingrese los datos del proveedor y los insumos solicitados."}
                        </p>
                    </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-5">
                            <div className="space-y-2">
                                ...
                                <input
                                    {...register("proveedor")}
                                    disabled={readOnly}
                                    className="w-full bg-[#1a1e2b] border border-[#1e2130] rounded-xl px-4 py-3 text-sm text-white focus:border-[#f97316] outline-none transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                            ...
                            <div className="space-y-2">
                                ...
                                <textarea
                                    ...
                                    rows={1}
                                    disabled={readOnly}
                                    className="w-full bg-[#1a1e2b] border border-[#1e2130] rounded-xl px-4 py-3 text-sm text-white focus:border-[#f97316] outline-none transition-all resize-none disabled:opacity-70 disabled:cursor-not-allowed"
                                    ...
                                    <div key={field.id} className="grid grid-cols-1 md:grid-cols-12 gap-3 p-4 rounded-xl bg-[#1a1e2b] border border-[#1e2130] relative group">
                                        <div className="md:col-span-6 space-y-1">
                                            ...
                                            <select
                                                ...
                                                disabled={readOnly}
                                                className="w-full bg-[#13161e] border border-[#1e2130] rounded-lg px-3 py-2 text-sm text-white focus:border-[#f97316] outline-none disabled:opacity-70 disabled:cursor-not-allowed"
                                            >
