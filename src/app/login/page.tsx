...
export default function LoginPage() {
    ...
    return (
        <div className="min-h-screen flex items-center justify-center px-4"
            style={{ background: AppColors.bg, fontFamily: "'DM Sans', system-ui, sans-serif" }}>
            ...
            <div className="w-full max-w-md">
                ...
                <div className="flex flex-col items-center mb-8">
                    <div className="w-16 h-16 rounded-2xl flex items-center justify-center mb-4"
                        style={{ background: AppColors.orange, boxShadow: `0 8px 32px ${AppColors.orange}50` }}>
                        <Factory className="w-8 h-8 text-white" />
                    </div>
                    <h1 className="text-2xl font-black text-white tracking-tight">Meme Fábricas</h1>
                    <p className="text-sm mt-1" style={{ color: AppColors.slate }}>Sistema de Control de Producción · IA</p>
                </div>
                ...
                <div className="rounded-2xl overflow-hidden" style={{ background: AppColors.surface, border: `1px solid ${AppColors.border}` }}>
                    ...
                    <form onSubmit={handleLogin} className="p-5 space-y-4">
                        ...
                        {error && (
                            <div className="flex items-center gap-2 px-4 py-3 rounded-xl text-sm"
                                style={{ background: `${AppColors.red}15`, border: `1px solid ${AppColors.red}40`, color: AppColors.red }}>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative">
                                <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: AppColors.slate }} />
                                <input type="text" value={usuario} onChange={e => { setUsuario(e.target.value); setError(""); }}
                                    ...
                                    style={{ background: "#0d1018", border: `1.5px solid ${usuario ? AppColors.orange : AppColors.border}` }} />
                            </div>
                        </div>
                        ...
                        <div className="space-y-1.5">
                            ...
                            <div className="relative">
                                <Lock className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: AppColors.slate }} />
                                <input type={verPass ? "text" : "password"} value={password}
                                    ...
                                    style={{ background: "#0d1018", border: `1.5px solid ${password ? AppColors.orange : AppColors.border}` }} />
                                <button type="button" onClick={() => setVerPass(v => !v)}
                                    className="absolute right-3 top-1/2 -translate-y-1/2 p-1" style={{ color: AppColors.slate }}>
                        ...
                        <button type="submit" disabled={cargando}
                            ...
                            style={{
                                height: 52,
                                background: cargando ? AppColors.slate : AppColors.orange,
                                boxShadow: cargando ? "none" : `0 4px 20px ${AppColors.orange}40`,
                    ...
                    {/* Footer de seguridad */}
                    <div className="px-5 py-3 border-t flex items-center gap-2" style={{ borderColor: AppColors.border }}>
                        <Lock className="w-3 h-3" style={{ color: AppColors.slate }} />
                        <a href="https://www.google.com/" className="text-xs" style={{ color: AppColors.slate }}>Olvidaste la contraseña?</a>
                    </div>
                </div>
